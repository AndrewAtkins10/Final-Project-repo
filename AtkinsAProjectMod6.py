...
def decrypt():
    try:   
        ...
        for num in decryptedList:
            try:
                ASCII = int(num) + count  # Validate the number can be converted
                password += chr(ASCII)
                count += 1
            except ValueError:
                print(f"Invalid number in encrypted file: '{num}'")
                continue
